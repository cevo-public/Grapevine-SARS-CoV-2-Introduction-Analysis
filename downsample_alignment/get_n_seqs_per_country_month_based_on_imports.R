...
if (N_CONTEXT_SEQS > 0) {
  ...
  cols <- c("country_recoded", "year_month", "n_arrivals", 
            "avg_daily_infectious_individuals	avg_population",
            "n_cases", "avg_per_pop_infectious", "est_n_imports", 
            "year_month_dec", "est_n_imports_padded", "n_seqs_ideal", 
            "n_seqs_available", "n_seqs_actual", "n_seqs_remaining",  
            "n_seqs_accounted_for")
  sampling_info_fill_backward <- as.data.frame(matrix(
    ncol = length(cols), nrow = 0))
  colnames(sampling_info_fill_backward) <- cols
}
