...
# as in a Swiss cluster (as defined in the cluster_data produced by 
# pick_swiss_clusters.R) are treated like tips in that their locations are fixed. 
...
# CONTEXT_METADATA <- paste(WORKDIR, "/test_cases/test_case_", TEST_CASE_NAME, "/context_metadata.txt", sep = "")
...
# CLUSTER_DATA <- paste(WORKDIR, "/test_cases/test_case_", TEST_CASE_NAME, "/results/", PREFIX, "_clusters.txt", sep = "")
# IS_VERBOSE <- F
# PLOT_TREE <- T
# WRITE_SCORES <- T
# IS_TEST_CASE <- T
...
if (!IS_TEST_CASE) {
  ...
  parser$add_argument("--clusters", type="character")
...
context_data_2 <- tree_data[tree_data$label %in% context_data$strain, "node"]
context_node_set <- context_data_2$node
print(paste(length(context_node_set), "context sequences"))
...
tree_data$asr_loc <- tree_data$country_recoded
tree_data[tree_data$node %in% cluster_data$ch_mrca, "asr_loc"] <- "Switzerland"
...
get_cluster_tips <- function(ch_mrca, cluster_data) {
  tips_str <- cluster_data[cluster_data$ch_mrca == ch_mrca, "tip_nodes"]
...
get_asr_scores <- function(node, n_tips, loc_to_idx_mapping, verbose) {
  ...
  if (is_ch_mrca(node = node, n_tips = n_tips)) {
    ...
    assign_node_as_swiss(node, cluster_tips = get_cluster_tips(ch_mrca = node, cluster_data = cluster_data), loc_to_idx_mapping = loc_to_idx_mapping, verbose)
    return()
  }
...
n_tips <- length(tree_ids)
candidate_asr_locs <- c(unique(context_data$country_recoded), "dummy_loc")  # include dummy location to keep track of the max. # changes required from any node
...
get_asr_scores(
  ...
  loc_to_idx_mapping = loc_to_idx_mapping, verbose = IS_VERBOSE)
...
min_scores <- unlist(lapply(
  X = asr_pie_data[, 1],
  FUN = get_n_descendent_swiss_mrcas,
  tree_data = tree_data, cluster_data = cluster_data, n_tips = n_tips))
...
tree_data_to_print <- merge(
  x = tree_data %>% select(-c("asr_loc", "candidate_asr_locs", "candidate_asr_scores", "candidate_asr_pointers")),
  y = asr_pie_data_to_merge, 
  by = "node")
...
if (PLOT_TREE) {
  ...
  tree_data <- tree_data %>% mutate(
    node_annotation = case_when(
      node %in% cluster_data$ch_mrca ~ "(Swiss)",
      T ~ ""),
    is_context  = case_when(
      node %in% context_node_set ~ "Context",
      T ~ "Priority"))
  ...
  p <- ggtree(tr = tree_2, size = 0.2) +
    geom_inset(insets = pies) +
    geom_tiplab(aes(
      label = ifelse(
        test = is.na(node_annotation),
        yes = "",
        no = paste(node, ": ", country_recoded, sep = "")),
  ...
  pdf(file = paste(OUTDIR, paste(PREFIX, "tree_with_asr.pdf", sep = "_"), sep = "/"), height = n_tips / 9, width = 13)
  show(p)
  dev.off()
}
